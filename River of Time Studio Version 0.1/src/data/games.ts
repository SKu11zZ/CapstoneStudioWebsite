export const games = [
  {
    id: "beyond-river",
    title: { zh: "长河彼岸", en: "Beyond the River of Time" },
    status: { zh: "原型开发中", en: "Prototype in Development" },
    genre: {
      zh: "叙事 RPG / 奇幻冒险 / 选择驱动",
      en: "Narrative RPG / Fantasy Adventure / Choice-Driven",
    },
    summary: {
      zh: "沿着贯穿时代的神秘长河，寻找被时间抹去的王国、亲人与真相",
      en: "Follow a mysterious river across eras to find a kingdom, a family, and a truth erased by time",
    },
    visual: "river",
  },
  {
    id: "stars-dream",
    title: { zh: "群星入梦", en: "Stars Into Dream" },
    status: { zh: "概念阶段", en: "Concept Phase" },
    genre: {
      zh: "梦境冒险 / 轻 RPG / 角色叙事",
      en: "Dream Adventure / Light RPG / Character Narrative",
    },
    summary: {
      zh: "进入角色的梦境星图，修复记忆裂缝，理解每个人未说出口的愿望",
      en: "Enter dream star maps, mend memory fractures, and understand the wishes each person never said aloud",
    },
    visual: "stars",
  },
  {
    id: "seventh-letter",
    title: { zh: "第七封信", en: "The Seventh Letter" },
    status: { zh: "早期构思", en: "Early Concept" },
    genre: {
      zh: "书信叙事 / 悬疑冒险 / 家族秘密",
      en: "Epistolary Narrative / Mystery Adventure / Family Secret",
    },
    summary: {
      zh: "通过跨越不同时代的七封信，拼合一个家族隐藏数代的真相",
      en: "Piece together a family secret hidden for generations through seven letters written across different eras",
    },
    visual: "letter",
  },
  {
    id: "bells-afar",
    title: { zh: "远方钟声", en: "Bells from Afar" },
    status: { zh: "设计验证中", en: "Design Validation" },
    genre: {
      zh: "旅行 RPG / 城镇故事 / 伙伴冒险",
      en: "Travel RPG / Town Stories / Companion Adventure",
    },
    summary: {
      zh: "穿过一座座即将被遗忘的城镇，记录人们听见最后钟声前的故事",
      en: "Travel through towns on the edge of being forgotten and record the stories people tell before the final bell",
    },
    visual: "bells",
  },
  {
    id: "lost-chronicles",
    title: { zh: "失落纪年", en: "Lost Chronicles" },
    status: { zh: "概念储备", en: "Concept Reserve" },
    genre: {
      zh: "史诗 RPG / 文明遗迹 / 多结局",
      en: "Epic RPG / Lost Civilizations / Multiple Endings",
    },
    summary: {
      zh: "在破碎的历史章节中行动，决定一个古老王国应该被复兴、遗忘，还是重新书写",
      en: "Move through broken chapters of history and decide whether an ancient kingdom should be restored, forgotten, or rewritten",
    },
    visual: "chronicle",
  },
];
